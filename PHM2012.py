...
os_dir2 = "/home/njh/PHM2012/Bearing1_1/"

file_name_all = os.listdir(os_dir2)
file_name_0_0 = []
for name in file_name_all:
    name_new = re.sub("[^0-9/./-]","",name)+'csv'
    os.rename(os_dir2+name,os_dir2+name_new)
...
#for name in range(0,1):
    ...
    #new data
    app_train = pd.read_csv(os_dir2+str(name+1).zfill(5)+'.csv', names =['Time1','Time2','Time3','Time4','v1','v2'])
    #app_train2.drop(['vibration_1'],axis=1,inplace = True)
    app_train.drop(['Time1'],axis=1,inplace = True)
    app_train.drop(['Time2'],axis=1,inplace = True)
    app_train.drop(['Time3'],axis=1,inplace = True)
    app_train.drop(['Time4'],axis=1,inplace = True)
    app_train.drop(['v1'],axis=1,inplace = True)
    ...
    #提取统计量
    plus_cells = int(app_train.shape[0]/1)
    for k in range(0,1):
    ...
    app_train_merge_sensor_m['RUL']  = 101.5 - 5 * name
...
app_train_merge_sensor.to_csv('/home/njh/final_data/'+'PHM2012_HHT01.csv',index='False')
